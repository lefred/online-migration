...
def get_common_objects_mig(db1_objects,db2_objects,print_list=True,options={}):
    in_both, in_db1_not_db2, in_db2_not_db1 = get_common_lists(db1_objects,
                                                               db2_objects)
    if print_list:
        print "FRED LET's DO IT" 
        server1_str = "server1." 
        server2_str = "server2." 
...
def _get_transform_mig(db_object, object1, object2, options):
    """Get the transformation SQL statements
...
def get_object_bef_mig(db_obj, object1, options):
    object1_create = get_create_object_mig(db_obj, object1, options)
    return object1_create

def diff_objects_mig(db_obj, object1_create, object2, options):
    ...
    db_obj[in]         database connection object
    server2[in]        second server connection
    object1            the first object in the compare in the form: (db.name)
    object2            the second object in the compare in the form: (db.name)
    options[in]        a dictionary containing the options for the operation:
    ...
    quiet = options.get("quiet", False)
    verbosity = options.get("verbosity", 0)
    difftype = options.get("difftype", "sql")
    width = options.get("width", 75)
    direction = options.get("changes-for", "server1")
    reverse = options.get("reverse", True)

    object2_create = get_create_object_mig(db_obj, object2, options)
    
    if not quiet:
        msg = "# Comparing {0}".format(object2)
        print msg,
    ...
    if direction == 'server1' or reverse:
        print "FRED WE ARE HERE !!"
        diff_server1 = _get_diff(object1_create_list,
                                 object2_create_list,
                                 object2, object2, difftype)
        # If there is a difference. Check for SQL output
        if difftype == 'sql' and len(diff_server1) > 0:
            transform_server1 = _get_transform_mig(db_obj,
                                               object1_create, object2, options)
#                                               object1_foreign_keys_bef, object2, options)

        # If there is a difference. Check for SQL output
        if difftype == 'sql' and len(diff_server2) > 0:
            transform_server2 = _get_transform_mig(db_obj,
                                               object1_foreign_keys_bef, object2, options)
